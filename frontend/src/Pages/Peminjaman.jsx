...
const Peminjaman = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      let idBuku = formData.idBuku;

      if (formData.isManualBuku) {
        const createBukuResponse = await createBukuTanah({
        ...
        idBuku = createBukuResponse.data?.data?.id_buku;

        if (!idBuku) {
          throw new Error("Gagal membuat data buku tanah");
        }
      }
